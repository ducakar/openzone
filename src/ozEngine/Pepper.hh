  ...
  void operator << ( Method method ) const
  {
    if( Pepper::isMainThread() ) {
      ...
      struct LambdaCallback
      {
        ...
        static void callback( int, void* data )
        {
          const LambdaCallback* lc = static_cast<const LambdaCallback*>( data );
          lc->func();
          Pepper::mainCallSemaphore.post();
        }
      };
      LambdaCallback lambdaCallback( method );

      Pepper::mainCall( LambdaCallback::callback, &lambdaCallback );
